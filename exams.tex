...
\tikzset{
  ...
  doorway/.style = {draw, rectangle, minimum height=2cm, text
    width=1.2cm},
...
\solution{
  ...
  %% Master thesis p.50.
  \begin{tikzpicture}
    \node[doorway] (async entry) [] {Asynch-ronous entry};
    \node[doorway] (sync entry)  [right=of async entry] {Synch-ronous entry}
    edge [pre] (async entry);
    \node[doorway] (async exit)  [right=of sync entry] {Asynch-ronous exit}
    edge [pre] (sync entry);
    \node[doorway] (dosome)  [right=of async exit] {Do some}
    edge [pre] (async exit);
    \node[doorway] (sync exit)  [right=of dosome] {Synch-ronous exit}
    edge [pre] (dosome)
...
\oklarhet{The answer is missing ``totally ordered''?}
%
...
\solution{
  ...
  In dining philosophers problem five or more philosophers (processes)
  dines together at a table. The philosophers has three states:
  \textit{thinking, hungry} and \textit{eating}.  Between each of the
  two philosophers lies one fork, i.e. the philosophers shares forks
  between each other.  In order for a philosopher to eat he must acquire
  both his left and his right fork. The main problem is to guarantee no
  deadlock, where no one can eat because no one has two forks, and no
  starvation, where one hungry philosopher never gets to eat.

  To solve this problem, the Chandy and Misra algorithm can be used,
  as described below:\\

  \usebox{\userinput}
  Time complexity:
  A process at depth $k$ will have to wait for forks from neighbors
  with lower precedence time $2(d+l)$ plus the time from neighbors of
  depth $k-1$ needs to be able to eat.

  From the correctness proof it can be concluded that a process at depth
  $k$ will have to wait for forks from neighbors with lower precedence
  time $2( d + l )$ plus the time neighbors of depth $k 1$ needs to be able
  to eat. This results in following recursion: ; $T ( k ) = T ( k ; 1) +
  4( d + l ) 4( k + 1)( d + l )$. Although the acyclic graph is initially
  of depth , the worst case depth is transformations of $H$ . Therefore, a
  process must wait time $O ( n ( d + l ))$.
}
...
\oklarhet{Unsure about the answer regarding avaiability in a and the
  whole answer b}
\solution[a]{Since one operation is executed at a time, the 3PC protocol must 
  finish before the next operation is performed. This makes the system
  rather inavailable, especially if many operations is executed.\\
  The system is both linerizable and sequentially consistent, because 
  all operations are done one at the time, and both the replicas updates
  in the same time.
}
\solution[b]{The system is not linearizable since the system lazily synchronizes
  the two servers. One of the criterias of linearizable is that the order
  of operations is the interleaving should be consistent with the real
  times at which the operations occurred in the actual execution. Because
  of the lazily synchronization there are no guarantee that this criteria
  will be met. Hence, it is not linearizable.

  The system is neither sequentially consistent. This because the criteria
  that the orders of operations in the interleaving should be consistent
  at the program order in which each client executed them is not met. Even
  though Eric only has one client, he can make updates to both servers
  and because of the lazily synchronization the criteria is not met. For
  example if the value of x is 0 and Eric set the value to 1 on server A
  and then directly afterwards read the value from server B, then this is
  still 0.
}
\solution[c]{
  See: \ref{2013-03:quorum}b)
}
\points{10}
...
before $m_2$.''}
\oklarhet{Unsure about the answer given in b)}
\solution[a]{
     \begin{description}
        \item[FIFO ordering] if a FE issues r then r', then any correct RM handles r before r'
        \item[Causal ordering] if r $\rightarrow$ r', then any correct RM handles r before r'
        \item[Total ordering] if a correct RM handles r before r', then any correct RM handles r before r'
     \end{description}
}
\solution[b]{The causal property states that if an operation r $\rightarrow$ r' then any 
  correct replica manager handles r before r'. Since $m1 \rightarrow m2$ and the 
  replica manager handles both $m1$ and $m2$ then the replica manager must process
  $m1$ before $m2$ if it is a correct process.
}
...
\begin{lrbox}{\userinput}
  \begin{minipage}{0.25\linewidth}
      \begin{tikzpicture}
        ...
        \draw (1.2, 1.2) circle (0.2cm);

        \draw (-0.3,-0.3) -- (-0.3,0.3);    
        \draw (-0.3,0.3) -- (0.3,0.3);
        \draw (0.3, 0.3) -- (0.3,0.9);
        \draw (0.3, 0.9) -- (1.5,0.9); 
        \draw (1.5, 0.9) -- (1.5, -0.3);
        \draw (1.5, -0.3) -- (-0.3,-0.3);
      \end{tikzpicture} 
    \captionof{figure}{Write 1}
  \end{minipage}
  %
  \begin{minipage}{0.25\linewidth}
    \centering
      \begin{tikzpicture}
        ...
        \draw (0.9, 1.5) -- (0.9,0.9);
        \draw (0.9, 0.9) -- (0.3,0.9); 
        \draw (0.3, 0.9) -- (0.3, -0.3);
        \draw (0.3, -0.3) -- (-0.3,-0.3);
      \end{tikzpicture} 
    \captionof{figure}{Read 1}
  \end{minipage}
  %
    \begin{minipage}{0.25\linewidth}
    \centering
      \begin{tikzpicture}
        ...
        \draw (-0.3,1.5) -- (1.5,1.5);    
        \draw (1.5, 1.5) -- (1.5,-0.3);
        \draw (1.5,-0.3) -- (0.9,-0.3);
        \draw (0.9,-0.3) -- (0.9,0.9); 
        \draw (0.9, 0.9) -- (-0.3, 0.9);
        \draw (-0.3,0.9) -- (-0.3,1.5);
      \end{tikzpicture} 
    \captionof{figure}{Read 2}
  \end{minipage}
  %
    \begin{minipage}{0.25\linewidth}
    \centering
      \begin{tikzpicture}
        ...
        \draw (-0.3,1.5) -- (1.5,1.5);    
        \draw (1.5 ,1.5) -- (1.5,0.9);
        \draw (1.5, 0.9) -- (0.9,0.9);
        \draw (0.9, 0.9) -- (0.9,0.3); 
        \draw (0.9, 0.3) -- (-0.3, 0.3);
        \draw (-0.3, 0.3) -- (-0.3,1.5);
      \end{tikzpicture} 
    \captionof{figure}{Write 2}
  \end{minipage}
\end{lrbox}
%
\solution[a]{
  Since one operation is executed at a time, the 2PC protocol must 
  finish before the next operation is performed. This makes the system
  rather inavailable, especially if many operations is executed. Also, 
  the fact that 2PC is a blocking protocol makes the protocol not suited 
  for availability.
}
\solution[b]{Write quorums must be greater than half the total number of 
  replicas, i.e. $W > \frac{9}{2} \implies W > 4 \implies W \geq 5$\\
  Read and write quorums must be greater than the total number of 
  replicas, i.e. $R + W > 9 \implies R > 9 - W \implies R > 4 \implies R \geq 5$\\ \\
  %
  \usebox{\userinput}
  Because of the constraint on the size of read and write quorums then
  at least on of the replicas contains updated information after each operation. 
  This makes the system both linearizable and sequentially consistent.\\\\
  %
  \begin{tikzpicture}
    \draw (0,0) circle (0.2cm);
    \draw (0,0.6) circle (0.2cm);
