...
\oklarhet{Unsure about the answer regarding avaiability in a and the
  whole answer b}
\solution{
  a) Since one operation is executed at a time, the 3PC protocol must 
  ...
  b) The system is not linearizable since the system lazily synchronizes
...
\solution{
  a) \begin{description}
     ...
     \end{description}
  b) The causal property states that if an operation r $\rightarrow$ r' then any 
...
%
\solution{
  a) Since one operation is executed at a time, the 2PC protocol must 
  finish before the next operation is performed. This makes the system
  ...
  for availability.\\ \\
  %
  b) Write quorums must be greater than half the total number of 
