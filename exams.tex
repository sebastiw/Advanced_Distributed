...
\points{5}
\question{Can Byzantine agreement be reached for 8 generals, with 3 of
  them faulty?}
\question{Can Byzantine agreement be reached for 8 generals, with 3 of
...
\points{10}
\question{Give the definitions of Linearizability and Sequential
  Consistency.}
\question{A correctness property $\rho$ is compositional if, whenever
...
\points{10}
\question{Describe an algorithm for leader election on a ring
  ...
  at any time.}
\question{Compute the time complexity and communication complexity of
  your algorithm.}
\question{Is it possible to design a symmetric algorithm for leader
  election? If yes, provide such an algorithm, if no provide a proof.}

\points{15}
\question{Eric wants to build a replicated storage system. In his
...
\question{Eric decides to change his design and uses now the gossip
...
\question{Eric decides to change his design and uses now nine replicas
