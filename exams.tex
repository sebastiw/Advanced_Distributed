...
\oklarhet{Unsure about the answer regarding availability in a) [b?
  //seba] and the whole answer b)}
\solution[a]{
  The all or nothing approach 3PC make use of makes the system rather
  inavailable.

  The system is both linearizable and sequentially consistent, because
  all operations are done one at the time, and both the replicas updates
  in the same time.
}
\solution[b]{
  The system is not linearizable since the system lazily synchronizes
